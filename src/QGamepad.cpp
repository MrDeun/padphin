...
SdlGamepad::SdlGamepad(SDL_Gamepad *gamepad, SDL_JoystickID instanceId,
                       QObject *parent)
    : QObject(parent), m_gamepad(gamepad), m_instanceId(instanceId) {
  const char *n = SDL_GetGamepadName(gamepad);
  m_name = n ? QString::fromUtf8(n) : QStringLiteral("Unknown Gamepad");
  m_type = static_cast<GamepadType>(SDL_GetGamepadType(gamepad));
}
...
bool SdlGamepad::isButtonPressed(SdlGamepad::Button button) const {
  if (!m_gamepad || !m_connected)
    return false;
  return SDL_GetGamepadButton(m_gamepad,
...
void SdlGamepad::handleAxisEvent(SDL_GamepadAxis axis, Sint16 rawValue) {
  ...
  // Update cached values + fire the grouped property-change signals QML binds
  // to.
  switch (axis) {
