...
public:
    // Mirrors SDL_GamepadButton as a Q_ENUM so QML can use e.g. SdlGamepad.ButtonSouth.
    // Extend with the remaining SDL_GAMEPAD_BUTTON_* values (paddles, misc, touchpad) if you need them.
    enum Button {
    ...
    explicit SdlGamepad(SDL_Gamepad *gamepad, SDL_JoystickID instanceId, QObject *parent = nullptr);
    ~SdlGamepad() override;
...
signals:
    ...
    void axisMoved(SdlGamepad::Axis axis, float value); // normalized -1..1 (0..1 for triggers)
...
private:
    ...
    static Button toButton(SDL_GamepadButton button) { return static_cast<Button>(button); }
